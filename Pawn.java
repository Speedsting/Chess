...
public class Pawn {
    ...
    public Pawn(int xLoc, int yLoc, int squareSize) {
        this.xLoc       = xLoc;
        this.yLoc       = yLoc;
        this.squareSize = squareSize;
        
        try {
            image = ImageIO.read(this.getClass().getResourceAsStream("Images/whitePawn.png"));
        } catch(Exception e) {}
    }
    ...
    public void move(int x, int y, Chess game) {
        ...
        if (canMove) {
            if (y == -1 && Math.abs(x) == 0)
                yLoc += y;
            else if (y == -2 && Math.abs(x) == 0 && yLoc == 6)
                yLoc += y;
    ...
    public void draw(Graphics g, Canvas c) {
        int xInPixels = (int)(squareSize * xLoc) + 84;
        int yInPixels = (int)(squareSize * yLoc) + 47;

        g.drawImage(image, xInPixels, yInPixels, c);
    }

    public void drawAtMouse(Graphics g, int xInPixels, int yInPixels, Canvas c) {
        g.drawImage(image, xInPixels - 25, yInPixels - 30, c);
    }
}